...
public class CodigoBarras2 {
    public static void ean8(int[] entry, int length, StringBuilder x){
        int counter = 0;
        boolean par = false;
        ...
        if((counter + entry[length-1])%10 == 0)
            x.append("SI\n");
        else
            x.append("NO\n");
    }

    public static void ean13(int[] entry, int length, StringBuilder x){
        int counter = 0;
        boolean par = false;
        for(int i=length-2; i>=0;i--){
            if(par){
    ...
    /**
     * Reads a line from stdin, stops when a non digit is found
     * @param ints Array to fill
     * @return Length of the string
     * @throws IOException
     */
    public static int readLine(int[] ints) throws IOException {
        int c = System.in.read();
        int currentIndex = 0;
        while(c >= '0' && c <= '9'){
            ints[currentIndex++] = c - '0';
            c = System.in.read();
        }
        return currentIndex;
    }
